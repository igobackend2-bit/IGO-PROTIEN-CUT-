...
export const PRODUCT_NAME_TRANSLATIONS: Record<
  string,
  { en: string; ta: string; hi: string; ml: string; te: string }
> = {
  'chk-01': {
 ...
 *
 * Tries an id-based lookup first (works for the bundled local/seed catalog),
 * then falls back to matching the exact English product name text (works
 * against the live Supabase catalog, whose rows have their own database ids
 * but were seeded with the same English names as the local catalog). Falls
 * back to the original name whenever the language is English or no
 * translation is found — so a live-catalog product added later by the admin
 * that isn't in this dictionary yet never renders blank, it just shows in
 * English until a translation is added here.
 */
const BY_NAME: Record<string, { en: string; ta: string; hi: string; ml: string; te: string }> = {};
