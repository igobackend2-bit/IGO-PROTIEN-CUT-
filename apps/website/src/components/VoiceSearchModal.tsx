...
export const VoiceSearchModal: React.FC<VoiceSearchModalProps> = ({
  ...
  // A "no-speech" right after the modal opens is very often just the
  // customer not being ready yet — the mic hardware is still spinning up
  // for the first ~200-400ms of `start()`, which eats into the browser's
  // short speech-detection window. Rather than dead-ending on the very
  // first attempt, retry once automatically, same as the network case.
  const retriedNoSpeechRef = useRef(false);
  ...
  const startRecognition = (langOverride?: string) => {
    ...
    recognition.onerror = (event: any) => {
      ...
      if (code === 'not-allowed' || code === 'permission-denied') {
        ...
        if (!retriedNoSpeechRef.current) {
          retriedNoSpeechRef.current = true;
          startRecognition(recognition.lang);
          return;
        }
  ...
  useEffect(() => {
    ...
    retriedNoSpeechRef.current = false;

    // A brief warm-up delay before the very first auto-start: calling
    // `start()` the instant the modal mounts means the mic hardware is
    // still spinning up during the first fraction of a second of the
    // browser's already-short speech-detection window — the customer
    // hasn't even had a chance to see "Listening..." before some of that
    // window is gone. A manual re-tap (mic button below) skips this delay
    // since the mic is already warm by then.
    const warmupTimer = setTimeout(() => startRecognition(), 350);
