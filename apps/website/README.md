...
> This is the **website** of a three-app monorepo. Read the [root README](../../README.md) first — in particular, this site runs on a different Supabase project and a different schema from the Flutter mobile app and admin dashboard, so its customers and orders are a separate dataset.

This README is written for whoever picks this codebase up next. The "Known Issues & What's Real vs. Mocked" section is the most important part — read it before you assume any number on the admin dashboard is live data.
...
src/
  ...
  context/        CartContext — cart, wishlist, delivery slot state (in-memory, see Known Issues)
...
- `supabase_setup.sql` — the schema file to run for **this website's** Supabase project. Creates `profiles`, `products`, `orders`, `delivery_slots`, `inbox_messages`, `customer_queries`, enables RLS on all of them, and defines the policies referenced above. Safe to re-run.
- `supabase_setup_website_variant.sql` — a second, drifted copy of the same schema that predated the monorepo merge. Diff it against `supabase_setup.sql` and reconcile before trusting either.
- The repo-root [`supabase/`](../../supabase) directory is a **different** backend — 18 migrations and 15 Edge Functions for the Flutter apps. It is not this website's schema; do not apply it to this project's database expecting the site to keep working.
