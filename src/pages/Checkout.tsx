    ...
    return (
      <div className="min-h-screen bg-neutral-light flex flex-col items-center justify-center px-6">
        ...
        >
          ...
          <p className="text-neutral-500 mb-2">Your order is confirmed.</p>
          ...
          <Link
            to="/"
            className="inline-flex items-center justify-center gap-2 w-full px-6 py-4 bg-igo-green text-white font-bold rounded-2xl hover:bg-igo-green/90 transition-all"
          >
            Continue Shopping
          </Link>
        </motion.div>
        ...
        <div className="flex items-center gap-2 mb-10 max-w-xs">
          {steps.map((step, i) => (
            <React.Fragment key={step}>
              ...
              >
                <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold border-2 ${i < currentStep ? 'bg-igo-green border-igo-green text-white' : i === currentStep ? 'border-igo-green text-igo-green' : 'border-neutral-200 text-neutral-300'}`}>
                  {i < currentStep ? <CheckCircle2 className="w-3.5 h-3.5" /> : i + 1}
                </div>
                <span className="text-xs font-bold hidden sm:inline">{step}</span>
              </div>
              {i < steps.length - 1 && <div className={`flex-1 h-0.5 ${i < currentStep ? 'bg-igo-green' : 'bg-neutral-200'}`} />}
            </React.Fragment>
          ))}
        </div>
                  ...
                  <div className="bg-white rounded-3xl p-6 sm:p-8 border border-neutral-100">
                    ...
                    <div className="flex gap-3 mb-6">
                      {[
                        ...
                        <button
                          key={id}
                          onClick={() => setAddress(a => ({ ...a, type: id }))}
                          className={`flex items-center gap-2 px-4 py-2 rounded-xl border-2 text-sm font-bold transition-all ${address.type === id ? 'border-igo-green text-igo-green bg-igo-green/5' : 'border-neutral-200 text-neutral-400'}`}
                        >
                  ...
                  <div className="bg-white rounded-3xl p-6 sm:p-8 border border-neutral-100">
                    ...
                    <div className="space-y-3">
                      {DELIVERY_SLOTS.map(({ id, label, icon: Icon, extra, note }) => (
                        <button
                          key={id}
                          onClick={() => { setSelectedSlot(id); setDeliverySlot(id); }}
                          className={`w-full flex items-center justify-between p-4 rounded-2xl border-2 transition-all text-left ${selectedSlot === id ? 'border-igo-green bg-igo-green/5' : 'border-neutral-100 hover:border-neutral-200'}`}
                        >
                          <div className="flex items-center gap-3">
                    ...
                    <div className="flex gap-3 mt-6">
                      <button onClick={() => setCurrentStep(0)} className="flex-1 py-4 border-2 border-neutral-200 rounded-2xl font-bold text-neutral-500 hover:bg-neutral-50 transition-all">
                        Back
                      </button>
                      <button
                        onClick={() => setCurrentStep(2)}
                        className="flex-1 py-4 bg-igo-green text-white font-bold rounded-2xl flex items-center justify-center gap-2 hover:bg-igo-green/90 transition-all shadow-lg shadow-igo-green/20 active:scale-[0.98]"
                      >
                  ...
                  <div className="bg-white rounded-3xl p-6 sm:p-8 border border-neutral-100">
                    ...
                    <div className="space-y-3 mb-6">
                      {PAYMENT_METHODS.map(({ id, label, icon: Icon, badge }) => (
                        <button
                          ...
                          className={`w-full flex items-center justify-between p-4 rounded-2xl border-2 transition-all ${paymentMethod === id ? 'border-igo-green bg-igo-green/5' : 'border-neutral-100 hover:border-neutral-200'}`}
                        >
                          <div className="flex items-center gap-3">
                          ...
                          {badge && (
                            <span className="text-xs font-bold bg-igo-green/10 text-igo-green px-2 py-1 rounded-lg">{badge}</span>
                          )}
                        </button>
                      ))}
                    </div>
                    ...
                    <div className="mb-6">
                      ...
                      <div className="flex gap-2">
                        ...
                        <button onClick={handleCouponApply} className="px-4 py-3 bg-neutral-dark text-white rounded-xl font-bold text-sm hover:bg-igo-green transition-all">
                          Apply
                        </button>
                      </div>
                    ...
                    <div className="flex gap-3">
                      <button onClick={() => setCurrentStep(1)} className="flex-1 py-4 border-2 border-neutral-200 rounded-2xl font-bold text-neutral-500 hover:bg-neutral-50 transition-all">
                        Back
                      </button>
                      <button
              ...
              <div className="space-y-3 mb-4 max-h-64 overflow-y-auto">
                {cart.map((item, i) => (
                  <div key={i} className="flex items-center gap-3">
                    <img
                      src={item.image || '/images/products/chicken-breast.png'}
                      alt={item.name}
                      loading="lazy"
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-neutral-400">{item.selectedWeight} × {item.quantity}</p>
                    </div>
